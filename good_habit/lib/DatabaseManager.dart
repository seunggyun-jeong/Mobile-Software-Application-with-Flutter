...
class HabitManager {
  ...
  HabitManager({required this.finishTime});
...
class DatabaseManager {
  ...
  void getDatabase() async {
    Future<Database> database = openDatabase(
      ...
      onCreate: (db, version) {
        return db.execute(
          "CREATE TABLE habit(finishTime INTEGER PRIMARY KEY, name TEXT, age INTEGER)",
        );
      },
      version: 1,
    );
  ...
  Future<void> insertHabit(HabitManager habit) async {
    // 데이터베이스 reference를 얻습니다.
    final Database db = await database;
